...
namespace BranteAccess.Speech
{
    ...
    /// automation objects needs: name-to-DISPID lookup, property get/put/putref, method calls, and
    /// VARIANT payloads of int / bool / string / IDispatch / byte-array (SAFEARRAY). Ported from
    /// wotr-access.
    /// </summary>
    internal sealed class ComDispatch : IDisposable
    {
        ...
        public void Dispose()
        {
            if (_ptr != IntPtr.Zero)
            {
                try { _release(_ptr); } catch (Exception ex) { HostLog.Warning("[speech] COM release failed: " + ex.Message); }
                _ptr = IntPtr.Zero;
            }
        }
