...
{
    /// <summary>
    /// Minimal loopback HTTP/1.1 server on a raw <see cref="TcpListener"/>. Speaks just enough of
    /// the protocol for curl (request line, Content-Length body, one response, connection close)
    /// and deliberately avoids System.Net.HttpListener so there is no http.sys URL-ACL / admin
    /// requirement. Loopback-only; a dev tool, not hardened. Ported from wotr-access. DEBUG-only.
    /// </summary>
    internal sealed class DevHttpServer
    {
        ...
        public void Start()
        {
            ...
            _listener.Start();
            _running = true;
            _thread = new Thread(Loop) { IsBackground = true, Name = "BranteDevHttp" };
            _thread.Start();
        }
        ...
        {
            _running = false;
            try { _listener?.Stop(); } catch (Exception ex) { HostLog.Warning("dev http stop: " + ex.Message); }
        }
        ...
        private void Loop()
        {
            while (_running)
            {
                ...
                {
                    if (_running) HostLog.Warning("dev http: " + e.Message);
                }
                finally
                {
                    if (client != null)
                    {
                        try { client.Close(); } catch (Exception e) { HostLog.Warning("dev http close: " + e.Message); }
                    }
                }
            }
        }
