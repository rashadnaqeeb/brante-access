...
namespace BranteAccess.Dev
{
    ...
    /// primitive: one instance holds what the mod has spoken (the driver can't hear TTS), another
    /// the mod's log lines. Callers poll with a cursor to get only what's new;
    /// <see cref="WaitForNew"/> long-polls so a driver can block instead of sleep-looping.
    /// Ported from Non-Visual Calculus.
    /// </summary>
    internal sealed class LineLog
    {
        ...
        /// <summary>The cursor one past the newest line (pass as since to get only the future).</summary>
        public long End
        {
        ...
        /// "index: text". <paramref name="next"/> is the cursor for only-newer next time.</summary>
        ...
        /// <summary>Block (on the calling HTTP thread) until a line with index &gt;=
        /// <paramref name="since"/> exists or the timeout passes. True if a line arrived.</summary>
