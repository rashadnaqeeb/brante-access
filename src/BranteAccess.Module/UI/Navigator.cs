...
namespace BranteAccess.Module.UI
{
    ...
    /// <see cref="GraphNavigator"/>, where announcements are PULL-based: focus is diffed per frame and
    /// a change speaks exactly once no matter what caused it - so implementations and screens never
    /// make per-callsite announce decisions. Ported from wotr-access.
    /// </summary>
    public abstract class Navigator
    {
        protected Screens.Screen Screen { get; set; }
        ...
        public abstract void Attach(Screens.Screen screen);

        /// <summary>Drop focus back to the screen's unfocused state - the same place Tab-off-the-end
        /// lands. On a <see cref="Screens.Screen.StartUnfocused"/> screen the keyboard returns to
        /// exploration and stays there; on other screens focus re-establishes next frame, so callers
        /// should only blur exploration-capable screens.</summary>
        ...
        /// its per-screen state so reopening starts fresh. Covered-but-alive screens keep theirs.</summary>
        public virtual void ScreenClosed(Screens.Screen screen) { }
        ...
        /// <summary>Move focus to the FIRST node of a Tab-stop (a screen seating a section whose node
        /// keys vary per state).</summary>
        ...
        {
            if (!string.IsNullOrEmpty(text)) Mod.Speech.Speak(text, interrupt);
        }
    }
}
