...
namespace BranteAccess.Module.UI.Graph
{
    ...
    /// rebuilds even when the world shifts under us. Ported from wotr-access (itself from Tanglebeep,
    /// with permission, which upgraded Factorio Access's plain string node key).
    ///
    /// <para><b>Reference</b> (optional) is the game/domain object a node was derived from (a Button,
    /// a MonoBehaviour, a save slot), compared by reference identity. <b>StructuralKey</b> (always
    /// present) is a value-equatable key - a string, or a composite such as a (region, row, col) tuple.</para>
    ///
