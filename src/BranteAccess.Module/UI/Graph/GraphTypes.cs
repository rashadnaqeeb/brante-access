...
namespace BranteAccess.Module.UI.Graph
{
    ...
    /// static edge can't express. Ported from wotr-access.</summary>
    ...
    /// ordering, node-over-type overriding, and (later) the user's per-kind announcement settings.</summary>
    ...
    /// <summary>
    /// One part of a control's spoken focus readout ("New Game" / "button" / "unavailable"), resolved
    ...
    public sealed class NodeAnnouncement
    {
        ...
        /// Kinds drive the control type's speak order and let a node's part override the type's common
        /// part of the same kind.</summary>
        public string Kind;
    ...
    /// <summary>
    /// A CONTROL TYPE - "button", "toggle", "slider" - as a registry value rather than a C# class. A
    /// type owns the speak ORDER of its announcement kinds and the parts COMMON to every control of the
    /// type (the localized role word); nodes contribute their specific parts, overriding a common part
    /// of the same kind.
    /// </summary>
    ...
    public sealed class NodeVtable
    {
        ...
        /// <summary>The control's type (registry value) - supplies the role word and the speak order.
        /// Null = an untyped one-off.</summary>
        ...
        /// <summary>Optional. Read / open the control's tooltip (Space). The action owns the whole
        ...
        /// wires these to a retained container's Expand/Collapse instead.</summary>
    ...
    /// change"). Kept as plain data; contextual announcements are composed from node metadata by the
    /// announcer, not per-edge closures (GC discipline).</summary>
    ...
    public sealed class GraphNode
    {
        ...
        /// log-like streams where "37 of 200" is noise (transcript lines, per DECISIONS.md).</summary>
