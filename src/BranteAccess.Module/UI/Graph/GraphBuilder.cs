...
{
    /// <summary>
    /// Builds a <see cref="GraphRender"/>. Ported from wotr-access. Two construction styles, freely
    ...
    /// structural level (announced when focus enters from outside), while <see cref="BeginGroup"/> pushes
    /// a focusable, EXPANDABLE group header (a tree section) whose children only emit while it's
    /// expanded - expansion state lives in the persistent set the builder is constructed with
    /// (<see cref="GraphState.Expanded"/>), so screens hold no tree state of their own. Nesting recurses;
    /// a collapsed ancestor suppresses everything beneath it.
    /// </summary>
    public sealed class GraphBuilder
    {
        ...
        // rows-then-raw (which would shove interleaved raw stops behind later buttons).
        ...
        /// <summary>Push one NON-FOCUSABLE level of presentation hierarchy onto nodes added from here -
        /// pure structure: never navigable, announced when focus enters from outside. Close with
        /// <see cref="PopContext"/>.</summary>
        ...
        /// state: <paramref name="expanded"/> when given (an adapter passes its retained state), else the
        /// persistent expansion set the builder was constructed with, else <paramref name="defaultExpanded"/>.
        /// The engine's tree operations (Right/Left) expand/collapse via the vtable's OnExpand/OnCollapse
        ...
        /// even BUILDING a collapsed group's children (a lazy hierarchy whose child data materializes on
        ...
        /// <summary>Directed edge <paramref name="from"/> to <paramref name="to"/>, with an optional spoken
        ...
        // can't cross. Stitch it: at each menu-to-raw boundary (declaration order, same stop), the menu
        // row's cells gain Down edges into the first raw node still missing an Up edge, and that node
        // gains the Up back; at raw-to-menu boundaries the reverse. Only MISSING edges are filled -
        // the raw content's own wiring is never overridden.
        private void StitchModeBoundaries()
        {
            ...
            foreach (var stop in stops)
            {
                ...
                for (int i = 1; i < nodes.Count; i++)
                {
                    ...
                    if (prevMenu) // menu row above raw content: row cells down into first raw node without an Up
                    {
                    ...
                    else // raw content above a menu row: last raw node without a Down wires to the row
                    {
                        var row = _rowOf[cur];
        ...
        // Auto-stamp "n of m" positions: a multi-item row's members are positioned within their ROW
        // (a bar); single-item-row nodes among the siblings sharing their (parent, stop) - the vertical
        // list/tree level arrows actually traverse. Raw/grid nodes get none. Announced only when m > 1
        // (a lone button reads no position).
