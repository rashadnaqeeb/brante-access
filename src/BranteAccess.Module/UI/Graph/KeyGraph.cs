...
namespace BranteAccess.Module.UI.Graph
{
    ...
    /// operation, with focus persisting in an external <see cref="GraphState"/>. Ported from
    /// wotr-access (itself from Tanglebeep, from Factorio Access's key-graph.lua). Two invariants:
    ///
    ...
    public sealed class KeyGraph
    {
        ...
        /// <summary>One step in <paramref name="dir"/>. Not moved (at an edge / empty) means To == From.</summary>
        ...
        /// announcement part - or null.</summary>
            ...
            foreach (var n in render.Order)
            {
                ...
                foreach (var a in anns)
                    if (a != null && a.Kind == AnnouncementKinds.Selected)
                    {
                        string t = null;
                        try { t = a.Text?.Invoke(); }
                        catch (Exception e) { Mod.Warn("selected-part probe threw on " + n.Id + ": " + e.Message); }
