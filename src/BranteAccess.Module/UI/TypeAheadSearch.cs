...
{
    /// <summary>
    /// Type-ahead search engine - ported from wotr-access (itself, with permission, from OniAccess's
    /// Handlers/TypeAheadSearch.cs). Speech and key routing live in <see cref="GraphNavigator"/>, which
    /// feeds typed characters and arrows in; this class is pure matching/result state.
    ///
    ...
    /// anywhere, then space-delimited word-prefix abbreviation ("ne ga" matches "new game"). Within a
    /// tier, items keep their LIST ORDER (the screen's element order: "l" must land by menu position,
    /// not name length), and matches in the item's NAME (before the first comma) rank ahead of matches
    /// in its appended metadata. Diacritics are ignored. Typing the same letter repeatedly cycles ALL
    /// of that letter's matches in list order (starts-with first, then the weaker tiers).
    /// </summary>
    public class TypeAheadSearch
    {
        ...
        /// 5 = space-delimited word-prefix abbreviation ("ne ga" in "new game").</summary>
