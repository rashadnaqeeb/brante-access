...
namespace BranteAccess.Module.Input
{
    ...
    /// key-repeat feels like the rest of the user's system. Queries user32
    /// SystemParametersInfo; where unavailable it falls back to sane defaults. Values are
    /// read once and cached (call Refresh to re-read). Ported from wotr-access.
    /// </summary>
    public static class OsKeyboard
    {
        ...
        private static void EnsureLoaded()
        {
            ...
            try
            {
                ...
                Mod.Log("OS key repeat query returned false; using defaults.");
            }
            catch (Exception e)
            {
                Mod.Log("OS key repeat unavailable, using defaults: " + e.Message);
            }
