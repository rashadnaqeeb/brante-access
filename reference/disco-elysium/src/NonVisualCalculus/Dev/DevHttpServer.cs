...
{
    /// <summary>
    /// Minimal loopback HTTP/1.1 server on a raw TcpListener. We speak just enough of the protocol for
    /// curl (request line, Content-Length body, one response, connection close) and deliberately avoid
    /// System.Net.HttpListener so there is no http.sys URL-ACL/admin requirement. Loopback-only; dev
    /// tool, not hardened.
    /// </summary>
    internal sealed class DevHttpServer
    {
        ...
        public DevHttpServer(int port, RequestHandler handler, Action<string> logWarn)
        {
            _port = port;
            _handler = handler;
            _logWarn = logWarn;
        ...
        public void Start()
        {
            ...
            _listener.Start();
            _running = true;
            _thread = new Thread(Loop) { IsBackground = true, Name = "NonVisualCalculusDevHttp" };
            _thread.Start();
        }
