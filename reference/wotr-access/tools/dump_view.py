"""Dump a view prefab's transform hierarchy from a UI bundle, by the script class on its root.
Usage: dump_view.py <bundle> <RootScriptClass> [maxdepth]"""
import UnityPy, os, sys
B=r"C:\Program Files (x86)\Steam\steamapps\common\Pathfinder Second Adventure\Bundles"
bundle, root_cls = sys.argv[1], sys.argv[2]
...
ms={};go_name={};go_tr={};tr_kids={};tr_go={};mb_go={}
...
def dump(gid,depth):
    ...
    while i<len(kids):
        ...
        if j-i>3: print("  "+"  "*(depth+1)+f"- [{knm} x{j-i}]"); i=j; continue
        if kg: dump(kg,depth+1)
        i+=1
roots=[gid for gid,_ in go_name.items() if root_cls in scripts(gid)]
for r in roots:
    print(f"### {go_name.get(r)} ({root_cls}) ###")
    dump(r,0)
